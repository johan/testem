...
App.prototype = {
  ...
  , initView: function(){
    ...
    if (self.onStartProcess){
      self.onStartProcess.bad(function(err, stdout, stderr){
        error( 'Error from on_start hook'
             , { error: err, stderr: stderr, stdout: stdout }
             )
  ...
  , configure: function(cb){
    ...
    config.read(function(){
      ...
      info( 'read configuration file'
          , { file: file, watch_files: watch_files }
          )
      fileWatcher.clear()
      if (file)
        fileWatcher.add(file)
      if (config.isCwdMode()){
        info('cwd mode - starting to watch ' + pwd)
        fileWatcher.add(pwd)
      ...
      config.getSrcFiles(function(err, files){
        var paths = files && _(files).pluck('src')
        info( 'config.getSrcFiles done'
            , { err: err
              , pwd: pwd
              , paths: paths
              , with_cb: typeof cb === 'function'
              }
            )
        self.watchFiles(paths)
      })
      if (cb) cb.call(self)
    })
  }
  ...
  , onFileRequested: function(filepath){
    if (!this.config.get('serve_files')){
      info('Started to watch just-accessed file ' + filepath)
...
function info() { return _log('info', arguments); }
function warn() { return _log('warn', arguments); }
function error() { return _log('error', arguments); }
function _log(level, args) {
  return log.log.apply(log, [level].concat([].slice.call(args, 0)))
}
